/*
 rpcc class handles client-side RPC functionalities.  Each rpcc is bound to a
 single RPC server.  The jobs of rpcc include maintaining connection with
 server, sending RPC requests and waiting for responses, retransmissions,
 at-most-once delivery etc.

 rpcs class handles server-side functionalities.  Each rpcs handles multiple
 ...
 Both rpcc and rpcs class use connection class as an abstraction for the
 underlying communication channel.  To send an RPC request/reply, one calls
 connection::send() which blocks until data is sent or connection has failed
 (thus caller can safely free the buffer occupied by send() arguments).  When a
 ...
 rely or error. All connections use a single PollMgr object to perform async
 ...
 port and a thread pool of x > 1 threads for executing RPC requests.  Using the
...
//make this upcall from connection object to 
//rpcc. 
...
{
	ScopedLock rwl(&reply_window_m_);
        std::list<reply_t>::iterator temp_reply;
       	
        std::list<reply_t> &temp_reply_list = reply_window_[clt_nonce];
 
        for(temp_reply = temp_reply_list.begin();temp_reply != temp_reply_list.end();temp_reply++)
        {
          if(temp_reply->xid == xid)
          {
            temp_reply -> cb_present = true;
            temp_reply -> buf = b;
            temp_reply -> sz = sz;
            return;
          }
...
{
	ScopedLock rwl(&reply_window_m_);
        //find the request in the request_window_
        std::map< unsigned int,std::list<reply_t> >::iterator temp_reply_pair;
        std::list<reply_t>::iterator temp_reply;
        std::list<reply_t>::iterator next_reply;
        temp_reply_pair = reply_window_.find(clt_nonce);
        
	if(temp_reply_pair != reply_window_.end())//find the client
        {          
          //trim the reply_window_ firstly
          for(temp_reply = temp_reply_pair->second.begin();temp_reply != temp_reply_pair->second.end();)
          { 
             if(temp_reply->xid <= xid_rep)
             {
              temp_reply_pair->second.erase(temp_reply);
              temp_reply = temp_reply_pair->second.begin();
              if(temp_reply_pair->second.begin() == temp_reply_pair->second.end())
               break;
             }
            else
            {
          ...
          if(xid <= xid_rep)
            return FORGOTTEN;
          //find whether there is xid match in the list
          for(temp_reply = temp_reply_pair->second.begin();temp_reply != temp_reply_pair->second.end();temp_reply++)
          {
            if(temp_reply->xid == xid)
            {
              if((temp_reply->sz != 0) || (temp_reply->buf != NULL))//the original request has been replied
              {
                //set the value of b and sz
                *b = (temp_reply->buf);
                *sz = (temp_reply->sz);
                return DONE;
              }
        ...
        //the request is new,two condition:1.the clt has been seen before;2.the clt is new
